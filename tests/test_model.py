...
def _training(rng, n=300):
    ...
    for i in range(n):
        ...
        x = [1.0, prior, 1.0, 1.0 if prior > 0 else -1.0, 1.0, 1.0,
...
def test_two_tier_routing_and_bounds():
    ...
    polled = _row("house", 2026, [1.0, 20.0, 1.0, 1.0, 1.0, 1.0, 22.0, 1.0], None, poll_count=4)
    unpolled = _row("house", 2026, [1.0, 20.0, 1.0, 1.0, 1.0, 1.0, 0.0, 0.0], None, poll_count=0)
    p_polled, p_unpolled = model.predict(polled), model.predict(unpolled)
    assert p_polled.model == "full" and p_unpolled.model == "fundamentals"
    ...
    no_prior = _row("house", 2026, [1.0, 0.0, 0.0, 1.0, 1.0, 1.0, 0.0, 0.0], None,
...
def test_forecast_payload_shape():
    ...
    row = _row("house", 2026, [1.0, -12.0, 1.0, -1.0, 1.0, 1.0, 0.0, 0.0], None)
...
def test_serialization_round_trip():
    ...
    row = _row("house", 2026, [1.0, 8.0, 1.0, 1.0, 1.0, 1.0, 0.0, 0.0], None)
